 ...
 * 只读取 {@link BladeHttpHeadersContextHolder} 这个 ThreadLocal，而该 ThreadLocal 在<b>同步请求</b>
 * 链路上没有任何地方写入（全项目仅异步 {@code BladeHttpHeadersCallable} 会 set），
 * 导致本服务发出的跨服务调用<b>不带 Blade-Auth</b>；下游带 {@code @PreAuth} 的接口会直接以
 * {@code SecureException: 请求未授权} 拒绝。</p>
 *
