...
-- 表名前缀：wf_*（已拍板；不使用 workflow_*，避免与  迁移源表 workflow_base 等撞名）
...
CREATE TABLE IF NOT EXISTS `wf_process_node` (
    ...
    `node_type`     TINYINT      NOT NULL                COMMENT '0创建 1审批 2提交 3归档 5等待 6自动处理（对齐  NodeType）',
...
CREATE TABLE IF NOT EXISTS `wf_node_field_perm` (
    ...
    `perm`          TINYINT      NOT NULL DEFAULT 2      COMMENT '0隐藏 1只读 2可编辑 3必填（对齐  fieldattr）',
    `tenant_id`     VARCHAR(32)  NOT NULL DEFAULT '000000' COMMENT '租户ID',
    `create_user`   BIGINT       NULL COMMENT '创建人',
    `create_dept`   BIGINT       NULL COMMENT '创建部门',
    `create_time`   DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
CREATE TABLE IF NOT EXISTS `wf_instance` (
    ...
    `update_user`       BIGINT       NULL COMMENT '修改人',
    `update_time`       DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
    `status`            INT          NOT NULL DEFAULT 0      COMMENT '0运行中 1通过 2不通过 3撤销 4暂停（对齐  currentstatus 语义）',
    `is_deleted`        INT          NOT NULL DEFAULT 0      COMMENT '逻辑删除',
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `wf_migration_map` (
    ...
    `ec_node_id`    BIGINT       NULL                      COMMENT ' 节点ID',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT=' 存量迁移映射';
...
-- 流程测试日志（对齐 ecology workflow_testlog）
-- 设计期「流程测试」产物：指定一个发起人，从创建节点出发按出口条件走查路径，
-- 逐节点解析操作者并记录过程，产出测试日志与结论。
-- ⚠️ 与 ecology 的差异（重要）：ecology 是「真实引擎 + 测试态标记(deleted=1)」；
--    本项目草稿流程尚未部署到 Flowable，且真实发起会污染 wf_instance / wf_task，
--    因此这里走**配置走查测试**：基于 wf_process_node / wf_node_link / wf_node_operator
--    的配置走路径、解析操作者，不落真实实例。
