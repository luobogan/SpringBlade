...
-- 1. 修改 workflow_bill 表：id 改为 BIGINT AUTO_INCREMENT
ALTER TABLE `workflow_bill` MODIFY COLUMN `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '表单ID';

-- 2. 修改 workflow_billfield 表：
--    id 改为 BIGINT AUTO_INCREMENT
--    billid 改为 BIGINT（匹配 form_definition.id 的 BIGINT 类型）
...
-- 3. 如果 workflow_billfield 已有数据，需要确认 billid 值不溢出
--    建议先清空测试数据再执行上述 ALTER

-- 4. 同样修复其他相关表（如有）
-- ALTER TABLE `workflow_formbase` MODIFY COLUMN `id` BIGINT NOT NULL AUTO_INCREMENT;
-- ALTER TABLE `workflow_formbase` MODIFY COLUMN `formid` BIGINT DEFAULT NULL;
